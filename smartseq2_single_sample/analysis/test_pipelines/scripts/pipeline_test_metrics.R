...
## ------------------------------------------------------------------------
load('~/Documents/HCA/metrics/metrics.RData')
ls()
...
pngfile<-paste('/Users/jishuxu//Documents/HCA/pipeline_test/metrics/plots/xyplot_',m,'_hisat2_star.png',sep='')
ggsave(p1,file=pngfile,type='cairo-png')
...
p
##hist
pngfile<-paste('/Users/jishuxu//Documents/HCA/pipeline_test/metrics/plots/hist_density_',m,'_hisat2_star.png',sep='')
xx<-data.frame('Run_s'=dd$Run_s,'hisat2'=dd[,j],'star'=dd[,i],'rlen'=dd$rlen)
xx.m<-melt(xx)
p<-ggplot(data=xx.m,aes(x=value,colour=paste(variable,paste(rlen,'bp',sep='')),linetype=paste(rlen,'bp',sep='')))
if(length(grep('PCT' , m))>1){
