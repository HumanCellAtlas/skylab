...
task QuantPairedEndNoBam {
  ...
  command {
    kallisto quant \
      ...
      --threads 4 \
      "${r1}" "${r2}" 
    mv abundance.h5 "${samplename}.abundance.h5"
    mv abundance.tsv "${samplename}.abundance.tsv"
    mv run_info.json "${samplename}.run_info.json"
  }
  ...
  output {
    File abundance_h5 = "${samplename}.abundance.h5"
    File abundance_tsv = "${samplename}.abundance.tsv"
    File log = "${samplename}.run_info.json"
  }
}
