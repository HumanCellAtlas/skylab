...
def add_cell_metrics(
    data_group, metrics_file, cell_ids, emptydrops_file, add_emptydrops_results,  verbose=False,
):
    ...
    data_group.create_dataset(
        "cell_metadata_float_name",
        shape=[final_df_float.shape[1], 1],
        compressor=COMPRESSOR,
        dtype=header_datatype,
        data=final_df_float.columns.astype(str),
    )
    data_group.create_dataset(
        "cell_metadata_float",
        shape=final_df_float.shape,
        compressor=COMPRESSOR,
        dtype=float_store_datatype,
        data=final_df_float.to_numpy(dtype=float_store_datatype),
    )
    if verbose:
        logging.info(
            "Added cell metadata_float with {} rows and {} columns".format(
    ...
    data_group.create_dataset(
        "cell_metadata_bool_name",
        shape=[final_df_bool.shape[1], 1],
        compressor=COMPRESSOR,
        dtype=header_datatype,
        data=final_df_bool_column_names,
    )
    data_group.create_dataset(
        "cell_metadata_bool",
        shape=final_df_bool.shape,
        compressor=COMPRESSOR,
...
def add_expression_counts(data_group, args):
    ...
    if args.verbose:
        logging.info(
            "shape of count matrix", exp_counts["shape"], csr_exp_counts.shape
        )
    # now create a dataset of zeros with the same dimensions as the expression count matrix
    exp_counts_group = data_group.zeros(
        "expression",
        compressor=COMPRESSOR,
    ...
    for i in range(0, csr_exp_counts.shape[0], CHUNK_ROW_SIZE):
        for j in range(0, csr_exp_counts.shape[1], CHUNK_COL_SIZE):
            ...
            # insert the chunk
            exp_counts_group[i:i + p, j:j + q] = csr_exp_counts[
                i:i + p, j:j + q
            ].toarray()
...
def create_zarr_files(args):
    ...
    add_cell_metrics(
        ...
        args.cell_metrics,
        cell_ids,
        args.empty_drops_file,
        args.add_emptydrops_results,
        args.verbose,
    )
