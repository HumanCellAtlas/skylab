...
1.1 Aligns reads to the genome with HISAT2 v.2.1.0
1.2 Calculates summary metrics from an aligned BAM using Picard

**Part 2: Transcriptome Quantification Modules**
2.1 Aligns reads to the transcriptome with HISAT v.2.1.0
2.2 Quantifies gene expression using RSEM v.1.3.0

## Part 1: Quality Control Modules
### 1.1 Align reads using HISAT2
HISAT2, a fast-paced, cost-efficient alignment tool, performs graph-based alignment of paired- or single-ended reads (in the form of fastq files) to a reference genome. For reference input, it requires an index which can be built following the [build_indices documentation](https://github.com/HumanCellAtlas/skylab/tree/master/library/accessory_workflows/build_indices) in skylab. HISAT2 can determine the presence of non-transcript sequences and true transcript sequences, taking into account the presence of single-nucleotide polymorphisms by using references such as NCBI's [dbSNP Build 150](https://www.ncbi.nlm.nih.gov/projects/SNP/snp_summary.cgi?view+summary=view+summary&build_id=150). The HISAT2 output is an aligned bam file. 

### 1.2 Calculate summary metrics using Picard
Quality control measurements are calculated using Picard tools, command line tools used for manipulating high-throughput sequencing data. Picard uses the aligned BAM file generated with HISAT2
