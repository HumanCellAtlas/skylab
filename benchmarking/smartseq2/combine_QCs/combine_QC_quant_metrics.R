...
library('rtracklayer')
library(optparse)
# blacklist, which are non-numerics QC metrics
BLACKLIST<-c('BAD_CYCLES','CATEGORY','LIBRARY','SAMPLE','READ_GROUP','READ_PAIRS','MEAN_READ_LENGTH','PAIR_ORIENTATION')
# calcaute detectable genes ratio
SummaryPerColumn<-function(cnts,threshold){
  cnt.dd<-cnts[,-c(1:2)]
  detected<-apply(cnt.dd,2,function(x){sum(x>threshold)})
...
ParseMTGene<-function(gtf_file,cnt){
  gtf_gencode <- readGFF(gtf_file, version=2L, tags = c("gene_name","gene_id", "transcript_id","gene_type"))
...
## params,python style 
option_list <- list(
  make_option("--datafile", type="character", default=NULL, 
  ...
  make_option("--metrics", type="character", default=NULL, 
              help=" metricsfile name", metavar="character"),
  make_option("--nthreshold", type="integer", default=10, 
              help=" threshold cut off in datafile", metavar="number"),
...
write.csv(met.core,file=paste(output_name,'_metrics_combined.csv',sep=''),
