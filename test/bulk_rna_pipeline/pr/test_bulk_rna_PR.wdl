...
workflow TestBulkRnaPR {
  ...
  call checker.ValidateBulkRna as checker {
    input:
      ...
      genome_flagstat = target.genome_flagstat[0],
      genomebam = target.genomebam[0],
      quants = target.kallisto_quants[0],
      mad_qc_metrics = target.madQCmetrics,
      rna_qc_metrics = target.rnaQC[0],
      rsem_genes_detected = target.number_of_genes[0],
